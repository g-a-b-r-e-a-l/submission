...
class Decoder(Model):
    ...
    def _train(self, train_X, train_Y):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx_t, batch_train in enumerate(tqdm(dataloader_train, desc=f"Training Epoch {epoch + 1}/{self.epochs}")):
                self.FP_model.train()
                self.NN_model.train()
                if batch_idx_t % 10 == 0:
...
class NeuralNetworkModel(nn.Module):
    ...
    def forward(self, spange_fp, res_time, temp, save_data):
        ...
        store, batch_idx, epoch, smiles = save_data
        ...
        if store:
            ...
            if len(smiles) != fp_projection.shape[0]:
                ...
                save_path = os.path.join(save_dir, f"projection_embeddings_batch_{idx}.json")
